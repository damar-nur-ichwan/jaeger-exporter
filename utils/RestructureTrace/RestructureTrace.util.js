...
function RestructureTrace(data=[]){
    data.map( span => {
        ...
        span.tags.forEach(tag => {
            if(tag.key === 'otel.status_code' || tag.key === 'span.kind' || tag.key === 'service.name' || tag.key === 'http.user_agent'){
    ...
    data.map( span => {
        if(span.references.length > 0) span['parentID'] = span.references[0].spanID
